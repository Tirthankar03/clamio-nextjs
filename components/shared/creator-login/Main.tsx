...
const MainContent = ({ isCardFocused }: any) => {
    return (
        <div className="p-6 space-y-6 md:space-y-0 md:grid md:grid-cols-4 md:gap-6">
            ...
            <div className={`flex flex-col items-center bg-secondary text-white p-6 rounded justify-center main-grid-4 ${isCardFocused ? 'focus-effect' : ''}`}>
                ...
                <Link href="/dashboard/create-product">
                    <button className="font-bold bg-primary text-secondary px-4 py-2 rounded mt-6 transition-colors duration-300 text-bold text-secondary">
                        Create Your Product
                    </button>
                </Link>
