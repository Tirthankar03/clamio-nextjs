      ...
      <div className="container mx-auto p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
          ...
          <div className="flex flex-col justify-between">
            <div>
              <h1 className="text-3xl font-bold mb-4">{ProdudctDetailData[0].productDetails.title}</h1>
              <div className="flex items-center mb-4">
                <FaUserCircle className="text-yellow-500 text-2xl" />
                <Link href={ProdudctDetailData[0].productDetails.creator.link} className="ml-2 text-blue-500 text-xl">{ProdudctDetailData[0].productDetails.creator.name}</Link>
              </div>
              <div className="flex items-center mb-4">
                <FaStar className="text-yellow-500 text-xl" />
                <span className="ml-2 text-xl">{ProdudctDetailData[0].productDetails.rating} ({ProdudctDetailData[0].productDetails.reviewsCount})</span>
              </div>
              <p className="mb-8">{ProdudctDetailData[0].productDetails.description}</p>
              <span className="text-2xl font-bold mb-4">${ProdudctDetailData[0].productDetails.price}</span>
            </div>
            <div className="flex my-5 md:my-0 lg:my-0 2xl:my-0 space-x-4">
              {ProdudctDetailData[0].productDetails.buttons.map((button, idx) => (
        ...
        <div className="grid grid-cols-2 py-5 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {ProdudctDetailData[0].highlights.map((highlight, idx) => (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {ProdudctDetailData[0].reviews.map((review, idx) => (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {ProdudctDetailData[0].similarProducts.map((product, idx) => (
            <div key={idx} className="bg-white p-4 shadow-lg rounded-lg relative">
              ...
              <div className="mt-4">
                ...
                <div className="flex justify-center items-center mt-2">
                  ...
                  <Link href={product.creator.link} className="ml-2 text-blue-500">{product.creator.name}</Link>
                </div>
